...
int create_threads(t_resources *resources)
{
    ...
    {
        if (pthread_create(&resources->threads[i], NULL, &philo_routine, &resources->philos[i]) != 0)
        {
    ...
    if (pthread_create(&resources->monitor_thread, NULL, &monitor_routine, resources) != 0)
    {
